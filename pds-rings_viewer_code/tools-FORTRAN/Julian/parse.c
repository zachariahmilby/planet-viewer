...
*       beginning with "MJD" is interpreted as a Modified Julian Date. You can 
*       also use "JED" and "MJED" to prefix strings given in ephemeris time.
*
...
*       1.1: Added JED and MJED time options.
*******************************************************************************/
...
* Modified Julian Date, possibly in ET instead of UTC.
*
* Input:
*       tokens[0...ntokens-1]   array of tokens to interpret.
*
...
RL_FLT8 *tai;
{
RL_INT4 isJD, isJED, isMJD, isMJED;
RL_FLT8 value, frac;
RL_BOOL status;
...
/* Test for JD or MJD prefix */
        ...
        isJED  = (strcmp(tokens[0].string, "JED")  == 0);
        isMJD  = (strcmp(tokens[0].string, "MJD")  == 0);
        isMJED = (strcmp(tokens[0].string, "MJED") == 0);
        if (!isJD && !isJED && !isMJD&& !isMJED) return FALSE;
...
/* Convert numeric value to TAI */
        if (isJD)       *tai = Jul_TAIofJD(value, JUL_UTC_TYPE);
        else if (isJED) *tai = Jul_TAIofJD(value, JUL_ET_TYPE);
        else if (isMJD) *tai = Jul_TAIofMJD(value, JUL_UTC_TYPE);
        else            *tai = Jul_TAIofMJD(value, JUL_ET_TYPE);
